...
export function ExperienceScene() {
  ...
  return (
    <section className="fade-in min-h-full py-10">
      <div className="mb-10">
        <p className="mb-3 font-mono text-sm uppercase tracking-[0.25em] text-cyan-300">{t("experience.eyebrow")}</p>
        <h2 className="text-4xl font-semibold md:text-6xl">{t("experience.title")}</h2>
      </div>
          ...
          <Panel key={`${item.company}-${item.role}`} className="p-5">
            <div className="grid gap-4 md:grid-cols-[180px_1fr]">
              <div className="font-mono text-sm text-cyan-300">
                {t("experience.step")} {String(index + 1).padStart(2, "0")}
              </div>
              <div>
